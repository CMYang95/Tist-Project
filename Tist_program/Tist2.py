...
with mp_pose.Pose(min_detection_confidence=0.8, min_tracking_confidence=0.8) as pose: 
    ...
    for timestamp in timestamps:
        ...
        try:
            ...
            #右手腕 #LEFT_WRIST
            right_wrist = [landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].y]
            #右手肩膀
            right_shoulder = [landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y]           
            #右手肘
            right_elbow = [landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].y]           
            #右邊hip
            right_hip = [landmarks[mp_pose.PoseLandmark.LEFT_HIP.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value].y]
            #右邊膝蓋
            right_knee = [landmarks[mp_pose.PoseLandmark.LEFT_KNEE.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value].y]
            #右邊腳踝
            right_ankle = [landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_ANKLE.value].y]  
    ...
    #肩膀平衡等等可改
        ...
        cv2.putText(image, "Bar  :"+str(Str_speed)+"m/s", #槓鈴速度
                    (45,100), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(0,36,255),1, cv2.LINE_AA)
        cv2.putText(image, f"Acceleration: {acceleration:.2f} m/s^2", 
                    (45,120), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 1)
        cv2.putText(image, f"Force: {force:.2f} N", 
                    (45,135), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 1)
        cv2.putText(image, "L_shoudler"+str(round(angle_of_right_shoulder,2))+"Dg", #右邊肩膀
                    (45,155),cv2.FONT_HERSHEY_SIMPLEX, 0.7,(0,36,255),1, cv2.LINE_AA)
        
        cv2.putText(image, "L_elbow"+str(round(angle_of_right_elbow,2))+"Dg", #右邊手肘
                    (45,185), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(0,36,255),1, cv2.LINE_AA)
        
        cv2.putText(image,"L_hip"+ str(round(angle_of_right_hip,2))+"Dg", #右邊hip
                    (45,215), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(0,36,255),1, cv2.LINE_AA)

        cv2.putText(image, "L_knee"+str(round(angle_of_right_Knee,2))+"Dg",#右邊膝蓋
                    (45,245), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(0,36,255),1, cv2.LINE_AA)
