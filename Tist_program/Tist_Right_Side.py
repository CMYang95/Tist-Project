...
all_time_angles=[]
all_time_speed=[]
def caulate_angle(a,b,c): #肩膀a 寬b 膝蓋c
    a=np.array(a) #第一個點
    b=np.array(b) #第二個點
    c=np.array(c) #第三個點
...
#抓鏡頭或是影片
cap= cv2.VideoCapture(r"D:\TIST_PROJECT\Tist_vide\SUN_SQ.mp4")      
#抓影片的大小fps等資訊
...
with mp_pose.Pose(min_detection_confidence=0.8, min_tracking_confidence=0.8) as pose: 
    ...
    #肩膀平衡等等可改
        ...
        cv2.putText(image, "Bar :"+str(Str_speed)+"m/s", #槓鈴速度
                    (39,100), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(255, 0, 255),1, cv2.LINE_AA)
        cv2.putText(image, f"Acceleration: {acceleration:.2f} m/s^2", 
                    (39,125), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 0, 255), 1)
        cv2.putText(image, f"Force: {force:.2f} N", 
                    (39,139), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (6255, 0, 255), 1)
        cv2.putText(image, "R-Shoudler"+str(round(angle_of_right_shoulder,2))+"Dg", #右邊肩膀
                    (39,158),cv2.FONT_HERSHEY_SIMPLEX, 0.7,(255, 0, 255),1, cv2.LINE_AA)
        
        cv2.putText(image, "R-Elbow"+str(round(angle_of_right_elbow,2))+"Dg", #右邊手肘
                    (39,189), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(255, 0, 255),1, cv2.LINE_AA)
        
        cv2.putText(image,"R-Hip"+ str(round(angle_of_right_hip,2))+"Dg", #右邊hip
                    (39,220), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(255, 0, 255),1, cv2.LINE_AA)

        cv2.putText(image, "R-Knee"+str(round(angle_of_right_Knee,2))+"Dg",#右邊膝蓋
                    (39,255), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(255, 0, 255),1, cv2.LINE_AA)
