...
#成功!!
cap = cv2.VideoCapture(r"D:\TIST_PROJECT\TIST_Cus_2023\cus7.mp4")
name_video="cus7"   #檔案命名
# YOLOv5 model
model = torch.hub.load('ultralytics/yolov5', 'custom', path=r'D:\TIST_PROJECT\Tist_program\plate.pt')
model.iou = 0.3
model.conf = 0.5
...
filename = os.path.join(r'D:\TIST_PROJECT\TIST_Cus_2023_output', name_video+time.strftime('-%H%M%S') + '.mp4')
...
with mp_pose.Pose(min_detection_confidence=0.8, min_tracking_confidence=0.8) as pose: 
    ...
    for timestamp in timestamps:
        ...
        cv2.putText(image, f"R-Shoulder:{angle_of_right_shoulder:.1f} deg ", #右邊肩膀
                    (39,160),cv2.FONT_HERSHEY_SIMPLEX, 0.8,(255,165,0),2, cv2.LINE_AA)
        
        cv2.putText(image, f"R-Elbow:{angle_of_right_elbow:.1f} deg ", #右邊手肘
                    (39,190),cv2.FONT_HERSHEY_SIMPLEX, 0.8,(255,165,0),2, cv2.LINE_AA)

        cv2.putText(image, f"R-Hip:{angle_of_right_hip:.1f} deg ", #右邊hip
                    (39,225),cv2.FONT_HERSHEY_SIMPLEX, 0.8,(255,165,0),2, cv2.LINE_AA)
        
        cv2.putText(image, f"R-Knee:{angle_of_right_Knee:.1f} deg ", #右邊膝蓋
        ...
        if cv2.waitKey(1) == ord('q'):
            break
plt.savefig(os.path.join('D:\TIST_PROJECT\Tist_outp', name_video+time.strftime('-%H%M%S'+'new')))
cap.release()
