...
path=[]
# Mouse callback function to select object to track
...
while True:
    ...
    if selected_object is not None:
        cv2.circle(frame, selected_object, 5, (0, 0, 255), -1)
        if len(trackers) == 0:
            x, y = selected_object
            tracker = dlib.correlation_tracker()
            tracker.start_track(frame, dlib.rectangle(x-10, y-10, x+10, y+10))
            trackers.append(tracker)
        selected_object = None
